...
const BaseLayout = () => {
  ...
  return (
    <div className='flex flex-col justify-between md:h-screen'>
      {/* <Navbar/> */}
      <div onMouseEnter={() => setValue(true)} onTouchStart={() => setValue(true)} onClick={() => setValue(true)} className={`   absolute bottom-4 right-4  z-40 flex items-center justify-center  hover:w-20 hover:h-20 w-10 h-10   transition-all duration-300 cursor-pointer  ${Value ? " max-md:w-20 max-md:h-20  " : "  "}  `}>
        <div onClick={() => { Value ? setValue(true) : console.log("not setting value") }} className={`" relative  z-[50] border border-black/20 font-poppins  rounded-full  hover:scale-[105%] transition-all duration-300  w-10 h-10  flex items-center justify-center cursor-pointer  ${Value ? " scale-[70%] bg-red text-white " : "bg-white text-black"} "`}>
          {!Value ? <div className=" -z-10  absolute  inline-flex h-full w-full animate-ping rounded-full bg-white opacity-asdas0 "></div>
            : null}
          {!Value ? "🕹️" : " X "}
